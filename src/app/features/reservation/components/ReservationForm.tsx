...
export default function ReservationForm({
    ...
    const targetStaff = useAppSelector((state) => state.user.users).find(
        (user) => user.id === formData.staffId
    )
    ...
    const hasDuplicatedReservation = useAppSelector(
        ...
        (reservation) =>
            ...
            reservation.staff.id === formData.staffId &&
            reservation.id !== reservationId
    )

    const isNotOnAShift = !useAppSelector((state) => state.shift.shifts).some(
        (shift) =>
            isAvailableAt(
                shift,
                formData.date,
                parseInt(formData.hour.split(':')[0])
            ) && shift.staffId === formData.staffId
    )
        ...
        <div className="min-h-screen bg-gray-50 px-4 py-6">
            <DatePickerModal
                ...
                currentSelection={{
                    ...
                    hour: parseInt(formData.hour.split(':')[0]),
                }}
                    ...
                    <div>
                        ...
                        <InputGroup>
                            <Form.Control
                                disabled
                                className={`px-4 py-3 ${
                                    hasDuplicatedReservation || isNotOnAShift
                                        ? 'bg-danger'
                    ...
                    {onDelete && (
                        <div className="flex gap-3">
                            <button
                                type="button"
                                onClick={onDelete}
                                className="flex-1 bg-red-500 text-white py-3 px-4 rounded-lg font-medium hover:bg-red-600 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 transition-colors"
                            >
                                削除
                            </button>
                        </div>
