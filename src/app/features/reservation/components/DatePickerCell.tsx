...
export default function DatePickerCell({
    ...
    const getReservations = (
        ...
        return reservations.filter((reservation) =>
            isReservedAt(reservation, day.format('YYYY-MM-DD'), hour)
        )
    }

    const hasReservations =
        getReservations(allReservations, day, hour).filter(
    ...
    const handleClick = () => {
        if (onCellClick) {
            onCellClick(
                ...
                hour,
                getReservations(allReservations, day, hour)
            )
        }
    }
    ...
    const renderContent = (hasReservations: boolean, hasShifts: boolean) => {
        if (
            currentSelection.day.isSame(day, 'day') &&
            currentSelection.hour === hour
        ) {
            return (
                <div className="align-items-center current-selection">
                    選択中
                </div>
            )
        }
