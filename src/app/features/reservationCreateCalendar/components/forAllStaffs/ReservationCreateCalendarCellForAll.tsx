...
const countAvailableStaffs = (
    reservationsAtTime: Reservation[],
    shiftsAtTime: Shift[]
): number => {
...
const getAvailableStaffs = (
    ...
    const reservationsAtHour = filterReservedAt(
        reservations,
        day.format('YYYY-MM-DD'),
        hour
    )
    const shiftsAtHour = filterShiftsAtHour(
        shifts,
        day.format('YYYY-MM-DD'),
        hour
    )
...
export default function ReservationCreateCalendarCellForAll({
    day,
    hour,
    allReservations,
    allShifts,
    onCellClick,
}: ReservationCreateCalendarCellForAllProps) {
    ...
    return (
        <td key={`${day.format()}-${hour}`} onClick={handleClick}>
            {availableStaffs === 0 ? (
                <div className="align-items-center bg-secondary-subtle">×</div>
            ) : (
